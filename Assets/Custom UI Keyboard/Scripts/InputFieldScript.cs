...
public class InputFieldScript : MonoBehaviour
{
    ...
    char chKorInput = ' '; //받은 글자
                           //kor UniCode = (초성 * 21 + 중성 )*28 + 종성 + 0xAC00

    //void OnClickedOnKor(TMP_InputField inputfiled, Text text)
    public void OnClickedOnKor(TMP_InputField inputfiled, string text)
    {
        //앞에 글자가 없거나 앞글자가 한글이 아닌경우 -> 그냥 바로 출력
        if (inputfiled.text.Length == 0 || !isKorean(inputfiled.text[inputfiled.text.Length - 1]) || !isKorean(text[0]))
        {
            string newText = text;
